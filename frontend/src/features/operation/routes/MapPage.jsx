...
import SvgMap from '../../location/components/SvgMap';
import LeafletMap from '../../location/components/LeafletMap';
import StatisticsPanel from '../../location/components/StatisticsPanel';
...
const createActiveObject = (optionsObj) => {
  const obj = {};
  Object.keys(optionsObj).forEach(key => { obj[key] = true; });
  return obj;
};

const getLeafletMapFilters = (activeEvidenceFilters) => {
  const lmFilters = {};
  ...
  if (activeMainCategory === 'all' || activeMainCategory === 'guns') {
    ...
    if (anyGunFilterActive) { lmFilters['gun'] = true; }
  }
  return lmFilters;
};

const getProvinceData = (evidenceData, filters) => {
  const provinceCount = {};
...
// --- Custom Hooks ---
function useEvidenceFilters(evidenceData) {
  ...
  return [evidenceFilterOptions, activeEvidenceFilters, setActiveEvidenceFilters];
}
...
function useLocationData() {
  ...
  const [loadingStates, setLoadingStates] = useState({
    provinces: true, districts: true, subdistricts: true,
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const provincesRes = await fetch(`${apiBase}/api/provinces`, { credentials: 'include' });
        const provincesData = await provincesRes.json();
        setProvinces(provincesData);
        setLoadingStates(prev => ({ ...prev, provinces: false }));

        const [districtsRes, subdistrictsRes] = await Promise.all([
          fetch(`${apiBase}/api/districts`, { credentials: 'include' }),
          fetch(`${apiBase}/api/subdistricts`, { credentials: 'include' }),
        ]);
        const districtsData = await districtsRes.json();
        const subdistrictsData = await subdistrictsRes.json();

        setDistricts(districtsData.map(d => ({ ...d, province_id: d.province_id || d.prov_id || null })));
        setLoadingStates(prev => ({ ...prev, districts: false }));

        setSubdistricts(subdistrictsData.map(sd => ({ ...sd, district_id: sd.district_id || null })));
  ...
  return { provinces, districts, subdistricts, loadingStates };
}
...
<StatisticsPanel
  ...
  isMobile={false}
  evidenceTypeFilter={activeEvidenceFilters.activeMainCategory}
/>
        </div>
      </div>
    </div>
    ...
    <div className="flex flex-col h-screen w-full relative">
      <div className="h-full w-full relative">
        ...
        <div className="absolute top-0 left-0 right-0 z-30 bg-white bg-opacity-90 p-2 shadow-md flex justify-between items-center">
          <div className="flex space-x-2"><button className={`px-3 py-1.5 text-xs font-medium rounded-md ${mapView === 'leaflet' ? 'bg-blue-600 text-white' : 'bg-gray-200'}`} onClick={() => setMapView('leaflet')}>แผนที่ภูมิศาสตร์</button><button className={`px-3 py-1.5 text-xs font-medium rounded-md ${mapView === 'svg' ? 'bg-blue-600 text-white' : 'bg-gray-200'}`} onClick={() => setMapView('svg')}>แผนที่แผนภาพ</button></div>
        ...
        {showStats && (
          <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-lg shadow-lg z-40 max-h-[70vh] overflow-y-auto transition-transform transform animate-slide-up">
            ...
            <button
              ...
              className="sticky bottom-0 w-full bg-gray-600 text-white py-2.5 text-sm "
            >
              ปิดสถิติ
            </button>
          </div>
...
const Map = () => {
  ...
  // Evidence filter state (custom hook)
  const [evidenceFilterOptions, activeEvidenceFilters, setActiveEvidenceFilters] = useEvidenceFilters(mockEvidenceData);

  // Location data (custom hook)
  const { provinces = [], districts = [], subdistricts = [], loadingStates = {} } = useLocationData();
  ...
  const leafletMapFilters = useMemo(() => {
    const lmFilters = {};
    // TODO: populate from activeEvidenceFilters if needed
    return lmFilters;
  ...
  const handleMainCategoryChange = useCallback((mainCategoryKey) => {
    setActiveEvidenceFilters(prev => {
      ...
      return {
        ...prev,
        activeMainCategory: mainCategoryKey,
        drugSubcategories: resetSubFilters(evidenceFilterOptions.drugSubcategories),
        drugStamps: resetSubFilters(evidenceFilterOptions.drugStamps),
        drugTypes: resetSubFilters(evidenceFilterOptions.drugTypes),
        drugCategories: resetSubFilters(evidenceFilterOptions.drugCategories),
        gunSubcategories: resetSubFilters(evidenceFilterOptions.gunSubcategories),
        gunMechanics: resetSubFilters(evidenceFilterOptions.gunMechanics),
        gunBrands: resetSubFilters(evidenceFilterOptions.gunBrands),
        gunModels: resetSubFilters(evidenceFilterOptions.gunModels),
      };
    });
  ...
  const handleDistrictSelect = useCallback((district, isCurrentlySelectedInCheckbox, forceSelect = false) => {
    setSelectedDistricts(prevSelectedDistricts => {
        ...
        if (!isAlreadyPresent || forceSelect) {
            if(!isAlreadyPresent) {
  ...
  if (!searchTerm.trim()) {
    ...
    })).sort((a, b) => a.province.province_name.localeCompare(b.province.province_name));
  }
  ...
  const sharedProps = {
    ...
    loading: overallLoadingForFilter,
  ...
  if (loadingStates.provinces && provinces.length === 0) {
