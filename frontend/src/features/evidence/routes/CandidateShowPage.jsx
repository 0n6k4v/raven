...
const FirearmCandidateCard = React.memo(({ candidate, selected = false, onClick }) => (
  <div
    className={`p-4 border border-gray-200 rounded-lg flex items-start cursor-pointer ${selected ? 'border-[#990000] bg-red-50' : ''}`}
    onClick={onClick}
    role="button"
  >
...
const GunBrandPanel = React.memo(({
  ...
  return (
    <div key={`brand-${brandIdx}`} className="border border-gray-200 rounded-lg overflow-hidden">
...
const CandidateShowPage = () => {
  ...
  return (
    <div className="flex flex-col h-screen max-h-screen bg-white">
      <div className="p-4 flex items-center border-b border-gray-200 shrink-0">
      ...
      {fullScreen && imageUrl && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex flex-col items-center justify-center z-50" role="dialog" aria-modal="true">
