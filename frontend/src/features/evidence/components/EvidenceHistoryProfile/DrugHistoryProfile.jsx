...
const ImageViewerModal = memo(({ isOpen, imageSrc, onClose }) => {
  ...
  return (
    <div 
      ...
      aria-modal="true"
    >
      <button onClick={onClose} className="absolute top-4 right-4 p-3 text-white/70 hover:text-white bg-white/10 hover:bg-white/20 rounded-full transition-all">
        <FiX size={24} />
      </button>
      <img src={imageSrc} alt="Full screen" className="max-w-[95vw] max-h-[95vh] object-contain shadow-2xl transition-transform duration-300 scale-100" onClick={(e) => e.stopPropagation()} />
    </div>
  );
});
...
const ConfidenceMeter = memo(({ confidencePercent, size = 24 }) => (
  <div className={`w-${size} h-${size} flex flex-col items-center justify-center`} aria-label="AI confidence meter" role="img">
    ...
    <p className="text-gray-500 text-[10px] text-center mt-1 font-medium leading-none">ความมั่นใจ AI</p>
  </div>
));

const DesktopLayout = memo(({ entity, onImageClick }) => {
  const narcotic = (Array.isArray(entity.exhibit.narcotics) ? entity.exhibit.narcotics[0] : entity.exhibit.narcotic) || {};
  ...
  return (
    <div className="hidden md:flex flex-row h-full w-full">
      <div className="w-1/2 p-6 flex justify-center items-center relative group border-r border-gray-100">
        <div className="relative cursor-zoom-in transition-transform hover:scale-[1.01]" onClick={() => onImageClick(entity.image)}>
          <img src={entity.image} alt={entity.altText} className="max-w-full h-auto object-contain max-h-96 rounded-lg shadow-sm" />
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/5 rounded-lg">
             <div className="bg-white/90 p-3 rounded-full shadow-lg text-gray-800"><FiZoomIn size={24} /></div>
          </div>
        </div>
      </div>
          ...
          <div className="mb-8">
            <div className="flex flex-row">
              <div className="space-y-4 w-2/3">
                ...
                <DetailRow label="ประเภท:">{narcotic.drug_type || entity.category}</DetailRow>
                <DetailRow label="หมวดหมู่:">{narcotic.drug_category}</DetailRow>
                <DetailRow label="น้ำหนัก:">{narcotic.weight_grams ? `${narcotic.weight_grams} กรัม` : '-'}</DetailRow>
                <DetailRow label="ลักษณะ:">{narcotic.characteristics}</DetailRow>
    ...
    <div className="flex md:hidden flex-col min-h-screen bg-white w-full">
      <div className="p-4 flex justify-center items-center bg-gray-50 border-b border-gray-100">
        <div className="relative" onClick={() => onImageClick(entity.image)}>
          ...
          <div className="absolute bottom-2 right-2 bg-white/90 p-2 rounded-full shadow-md text-gray-700"><FiZoomIn size={18} /></div>
        </div>
      </div>
      ...
      <div className="px-4 pt-6 pb-24">
        {/* ✅ ปรับใหม่: แบ่งสัดส่วนรายละเอียดกับ Meter ไม่ให้ทับกัน */}
        ...
        {/* ส่วนอื่นๆ แสดงเต็มความกว้างปกติ */}
...
const DrugHistoryProfile = memo(function DrugHistoryProfile({ item: rawItem }) {
...
DrugHistoryProfile.propTypes = { item: PropTypes.object.isRequired };
